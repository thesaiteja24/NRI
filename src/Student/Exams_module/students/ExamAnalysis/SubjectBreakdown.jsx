...
const SubjectBreakdown = ({ subjectBreakdown }) => {
  ...
  const showCodingSections = selectedData.coding.total > 0 || selectedData.coding.score > 0;
  ...
  return (
    <div className="w-full flex justify-center py-10 font-[Inter]">
      <div className="w-full max-w-full bg-white shadow-[0px_4px_20px_#B3BAF7] rounded-[20px] p-6 md:p-10 space-y-6">
        {/* Heading */}
        <h2 className="text-[25px] leading-[30px] font-semibold text-[#00007F]">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          <div className="text-[#00007F] font-medium text-[18.8px] leading-[66px]">
            Select Subject
          </div>
          ...
          <div className="min-w-[600px] bg-[#EFF3FF] border border-[#00007F] rounded-[20px] p-4 relative">
            <div className={`grid ${gridColsClass} text-center`}>
              ...
              <div className="px-4 flex flex-col  justify-between   items-center">
                ...
                <p className="text-[32px] font-bold text-black">{selectedData.attempted}</p>
              </div>
              {/* Column 2: MCQ Score */}
              <div className="px-4 flex flex-col justify-between   items-center">
                <p className="text-[15px] font-medium text-black">MCQ Score</p>
                <p className="text-[32px] font-bold text-black">{selectedData.mcq.score}</p>
              </div>
              ...
              <div className="px-4 flex flex-col justify-between  items-center">
                ...
                <p className="text-[32px] font-bold text-black">{selectedData.mcq.total}</p>
              </div>
              ...
              {showCodingSections && (
                <>
                  ...
                  <div className="px-4 flex flex-col  justify-between  items-center">
                    <p className="text-[15px] font-medium text-black">Total Coding Questions</p>
                    <p className="text-[32px] font-bold text-black">{selectedData.coding.total}</p>
                  </div>
              ...
              {Array.from({ length: dividerCount }).map((_, index) => (
                <div
                  key={`divider-${index}`}
                  className="absolute top-0 bottom-0 w-[1px] bg-[#00007F]"
                  style={{
