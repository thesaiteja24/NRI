  ...
  if (!details || !Array.isArray(details) || details.length === 0) {
    return (
      <div className="bg-white border border-[#19216f] rounded-lg shadow-md p-8 mt-4 text-xl">
        ...
        <p className="text-center text-gray-500">You have not attempted any questions.</p>
      </div>
    );
  }
          ...
          {details.map((question, index) => {
            ...
            const type =
              question?.options && Object.keys(question.options).length > 0
                ? "MCQ"
                : "Coding";
            const status = question.hasOwnProperty("status") ? question.status : "Not Attempted";
            ...
            const sourceCode = submitted?.sourceCode || question?.sourceCode || "";
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-4 flex-wrap">
                        <span className={correctAnswer === "A" ? "text-green-600 font-bold" : ""}>
                          A: {options.A || "N/A"}
                        </span>
                        <span className={correctAnswer === "B" ? "text-green-600 font-bold" : ""}>
                          B: {options.B || "N/A"}
                        </span>
                      </div>
                      <div className="flex gap-4 flex-wrap">
                        <span className={correctAnswer === "C" ? "text-green-600 font-bold" : ""}>
                          C: {options.C || "N/A"}
                        </span>
                        <span className={correctAnswer === "D" ? "text-green-600 font-bold" : ""}>
                          D: {options.D || "N/A"}
                        </span>
                      </div>
                    </div>
                            ...
                            <CodeMirror
                              ...
                              extensions={
                                languageExtensions[language]
                                  ? [languageExtensions[language], EditorView.editable.of(false)]
