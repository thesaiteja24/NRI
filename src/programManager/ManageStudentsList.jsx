...
export default function StudentsList() {
  ...
  // If needed, fetch batches when the component mounts
  useEffect(() => {
  ...
  // Replace the Batch No text input with a dropdown
  ...
  const filteredStudents = (studentsList || []).filter(student => {
    ...
    // Convert comma-separated input to array for departments.
    ...
    return (
      ...
      // For Batch, we now do an exact match since it's coming from a dropdown.
      (searchBatchNo === '' ||
        (student?.BatchNo || '').toLowerCase() === searchBatchNo.toLowerCase()) &&
      departmentMatch &&
      (searchHighestGraduation === '' ||
        (student?.qualification || '').toLowerCase().includes(searchHighestGraduation.toLowerCase())) &&
  ...
  return (
    <div className=" flex flex-col mx-auto p-6 min-h-[89vh] mt-0">
