...
const StudentAttendanceData = () => {
  const [selectedLocation, setSelectedLocation] = useState("NRIA");
  const [selectedTechStack, setSelectedTechStack] = useState("SelectTechStack");
  const [availableTechStacks, setAvailableTechStacks] = useState(techStackSubjects.NRIA);
  ...
  useEffect(() => {
    ...
    if (isAdmin || isAllLocation) {
      ...
      setSelectedLocation("NRIA"); // Default to NRIA for admin users
    } else {
      setSelectedLocation("NRIA"); // Hardcode to NRIA for non-admin users
      fetchBatches("NRIA");
    }
  }, [fetchBatches]);

  useEffect(() => {
    if (selectedLocation === "NRIA") {
      fetchBatches("NRIA");
      setAvailableTechStacks(techStackSubjects.NRIA);
    } else {
  ...
  const fetchAttendanceData = useCallback(async () => {
    if (selectedLocation !== "NRIA" || selectedBatch === "SelectBatch") {
      setAttendanceRecords([]);
      setCourseWiseData({});
      return;
    }
  ...
  return (
    <div className="bg-gradient-to-b p-4 md:p-6 mt-0 font-[inter]">
      <h1 className="text-2xl md:text-3xl font-medium text-center text-gray-800 mb-4 md:mb-8">
        <span className="text-black bg-clip-text">Student Attendance - NRIA</span>
      </h1>
