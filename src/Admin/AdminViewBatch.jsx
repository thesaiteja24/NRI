...
const AdminViewBatch = () => {
  ...
  const sessionStorageLocation = decryptData(sessionStorage.getItem("location")) || "all";
  ...
  }, [fetchBatches, sessionStorageLocation, userType]);
  ...
  }, [batches, locationFilter, userType]);
    ...
    <div className="flex flex-col items-center p-3 mt-0">
      <h1 className="text-4xl font-extrabold text-center text-gray-800 mb-2">
        <span className="text-black bg-clip-text">
          {userType
            ? `${userType === "Python" ? "Python Full Stack (PFS)" : "Java Full Stack (JFS)"} Batches`
            : "View & Edit Batches"}
        </span>
      </h1>
      ...
      {filteredBatches.length === 0 ? (
        <p className="text-center text-gray-600">
          {userType ? `No ${userType === "Python" ? "PFS" : "JFS"} batches found.` : "No batches found."}
        </p>
      ) : (
        <>
