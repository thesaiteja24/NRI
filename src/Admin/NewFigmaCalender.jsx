  ...
  return (
    <div className="flex flex-col justify-between h-full min-w-[90%] sm:min-w-[20%] lg:min-w-[15%] xl:min-w-0 snap-center overflow-hidden">
      ...
      <div className="flex flex-col items-center mt-3">
        ...
        <span className="text-[12px] leading-[15px] text-black font-normal mt-1">{label}</span>
      </div>
    </div>
  );
};

const NewFigmaCalender = ({ onDayClick, selectedDate }) => {
  ...
  const scroll = (dir) => {
    if (monthRef.current) {
      const cardWidth = monthRef.current.querySelector(".snap-center").offsetWidth;
      const cardsPerView = window.innerWidth >= 1024 ? 6 : window.innerWidth >= 768 ? 5 : 1;
    ...
    <div className="flex mt-4 mb-4 font-[Inter]">
      <div className="flex flex-col flex-grow min-h-[273px] bg-white rounded-md shadow-[0px_4px_17px_rgba(19,46,224,0.2)] p-4 sm:p-6 w-full">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-3 items-center">
            ...
            <div className="relative">
              ...
              >
                <span className="text-xs font-medium text-[#434343]">{selectedYear}</span>
                <ChevronDown size={12} color="#434343" strokeWidth={1.5} className="border border-[#434343] rounded-sm" />
              </button>
              {isYearDropdownOpen && (
        ...
        <div className="flex justify-between items-center mt-4 mb-2 xl:hidden">
          <button onClick={() => scroll("left")} className="p-1 rounded bg-gray-200">
            <ChevronLeft size={18} />
          </button>
          <button onClick={() => scroll("right")} className="p-1 rounded bg-gray-200">
...
export default NewFigmaCalender;