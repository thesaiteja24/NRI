...
const AdminStudentAttendanceData = () => {
  ...
  // Fetch batches based on selectedLocation
  useEffect(() => {
  ...
  }, [fetchBatches, userType, selectedLocation]);
  ...
  useEffect(() => {
    ...
    let filtered = batches.filter(batch => {
      ...
      if (!batch.Batch.startsWith(batchPrefix)) {
        console.warn(`Invalid batch prefix for ${userType}: ${batch.Batch} (ID: ${batch.id})`);
        return false;
      }
      return true;
    });
    ...
    setSelectedBatch("SelectBatch");
    setSelectedCourse("AllCourses");
  }, [batches, userType, selectedLocation]);
  ...
  const fetchAttendanceData = useCallback(async () => {
    if (selectedBatch === "SelectBatch" || !userType || selectedLocation === "SelectLocation") {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/batchwiseattends`, {
        params: {
          batch: selectedBatch,
          location: selectedLocation || "vijayawada",
        },
      });
  ...
  const handleExportAllToExcel = () => {
    ...
    XLSX.writeFile(wb, `All_Courses_Attendance_${selectedBatch}_${userType === "Python" ? "PFS" : "JFS"}_${selectedLocation || "vijayawada"}.xlsx`);
  };
    ...
    <div className="bg-gradient-to-b p-4 md:p-6 mt-0 font-[inter]">
      <h1 className="text-2xl md:text-3xl font-medium text-center text-gray-800 mb-4 md:mb-8">
        <span className="text-black bg-clip-text">
          {userType
            ? `${userType === "Python" ? "Python Full Stack (PFS)" : "Java Full Stack (JFS)"} Attendance`
            : "Student Attendance"}
        </span>
      </h1>
          ...
          {loading || batchesLoading ? (
            ...
            <p className="text-gray-500 font-semibold mt-4 md:mt-6">
              {userType && selectedLocation !== "SelectLocation"
                ? `No ${userType === "Python" ? "PFS" : "JFS"} attendance records found for ${selectedLocation.charAt(0).toUpperCase() + selectedLocation.slice(1)}.`
                : `No ${userType === "Python" ? "PFS" : "JFS"} attendance records found for Vijayawada.`}
            </p>
          ) : (
...
export default AdminStudentAttendanceData;